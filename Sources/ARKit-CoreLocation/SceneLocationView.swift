...
public extension SceneLocationView {
	...
	func addRoute(with coordinates: [CLLocationCoordinate2D], boxBuilder: @escaping BoxBuilder) {
		...
		if let flattened = sceneNode?.flattenedClone() {
			...
			/*
			let points = polyNode.locationNodes.map { $0.position }.filter({ !$0.isZero() })
			print(points)
			let lineNode = SCNLineNode(with: points, radius: 0.5, edges: 12, maxTurning: 16)
			lineNode.position = flattened.position
			lineNode.geometry?.firstMaterial?.blendMode = .screen
			removeAllNodes()
			removeAllRoutes()
